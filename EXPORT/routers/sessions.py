"""
Sessions API - Endpoints pour les onglets Générale, Détails Site et Analyse Erreur
"""
...
SITE_COLOR_PALETTE = [
    "#f97316", "#a855f7", "#eab308", "#ef4444", "#6366f1",
    "#3b82f6", "#06b6d4", "#10b981", "#0ea5e9", "#f43f5e",
]
...
def _build_conditions(sites: str, date_debut: date | None, date_fin: date | None, table_alias: str | None = None):
    """Construit les conditions WHERE pour les requêtes SQL"""
    conditions = ["1=1"]
    params = {}
...
def _apply_status_filters(df: pd.DataFrame, error_type_list: list[str], moment_list: list[str]) -> pd.DataFrame:
    """Applique les filtres de statut aux données"""
...
def _map_moment_label(val: int) -> str:
    """Convertit un code step en label de moment"""
    try:
        v = int(val)
    except Exception:
        return "Unknown"
...
def _build_pivot_table(detail_df: pd.DataFrame, by_site: pd.DataFrame) -> dict[str, Any]:
    """Construit une table pivot pour le détail par site"""
    if detail_df.empty:
        return {"columns": [], "rows": []}
...
def _prepare_query_params(request: Request) -> str:
    """Prépare les paramètres de requête pour les URLs"""
...
async def get_sessions_general(
    ...
    """Retourne les données pour l'onglet Générale"""
    ...
    if df.empty:
        return templates.TemplateResponse(
            "partials/sessions_general.html",
            {
                "request": request,
                "total": 0, "ok": 0, "nok": 0,
                "taux_reussite": 0, "taux_echec": 0,
                "recap_columns": [], "recap_rows": [],
                "moment_distribution": [], "moment_total_errors": 0,
                "error_type_distribution": [], "error_type_total": 0,
                "site_success_cards": [], "site_success_bars": [],
            },
        )
    ...
    if not err.empty:
        ...
        recap_columns = ["Site / PDC", "Total", "Total_OK", "Total_NOK"] + moment_cols + ["% OK", "% NOK"]
        ...
        pdc_recap = (
            stats_pdc.rename(columns={"total": "Total", "ok": "Total_OK", "nok": "Total_NOK", "taux_ok": "% OK"})
...
async def get_sessions_site_details(
    ...
    """Retourne les données pour l'onglet Détails Site"""
    error_type_list = [e.strip() for e in error_types.split(",") if e.strip()] if error_types else []
    moment_list = [m.strip() for m in moments.split(",") if m.strip()] if moments else []
    ...
    err_rows = df_site[~df_site["is_ok"]].copy()
    ...
    if not err_rows.empty:
        ...
        error_type_order = ["Erreur_EVI", "Erreur_DownStream", "Erreur_Unknow_S"]
        error_type_labels = {"Erreur_EVI": "EVI", "Erreur_DownStream": "Downstream", "Erreur_Unknow_S": "Erreur_Unknow_S"}
    ...
    if not err_rows.empty:
        ...
        if need_cols_ds.issubset(err_rows.columns):
            ...
            if not sub.empty:
                ...
                table["Percent"] = np.where(total_all > 0, (table["Total"] / total_all * 100).round(2), 0.0)
                table.insert(0, "Rank", range(1, len(table) + 1))

                total_row = {"Rank": "", "Code_PC": "Total", **{m: int(table[m].sum()) for m in downstream_moments}}
    ...
    if not err_rows.empty:
        ...
        if need_cols_evi.issubset(err_rows.columns):
            ...
            if not sub.empty:
                ...
                total_row = {"Rank": "", "EVI_Code": "Total", **{m: int(table[m].sum()) for m in evi_occ_moments}}
                total_row["Total"] = int(table["Total"].sum())
                total_row["Percent"] = 100.0 if total_all else 0.0
...
# ==========================================================================
# Endpoint: Analyse Erreur (Top 3 EVI + Downstream)
# ==========================================================================
...
async def get_error_analysis(
    ...
    """Retourne les données pour l'onglet Analyse Erreur - Top 3"""
    error_type_list = [e.strip() for e in error_types.split(",") if e.strip()] if error_types else []
    moment_list = [m.strip() for m in moments.split(",") if m.strip()] if moments else []
