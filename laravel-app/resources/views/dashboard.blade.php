...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Main */
        .main { max-width: 1750px; margin: 0 auto; padding: 1.5rem; }
        ...
        .kpi-card[data-nav-tab] { cursor: pointer; }
        .kpi-card::before {
            content: '';
            position: absolute;
            top: 0;
        ...
        .bar-success { background: linear-gradient(90deg, #86efac, #10b981); }
        ...
        /* Site expander */
        .site-expander { margin-bottom: 0.5rem; }
        .site-expander summary {
            display: flex;
            justify-content: space-between;
            align-items: center;
            padding: 0.75rem 1rem;
        ...
        .site-expander summary::-webkit-details-marker { display: none; }
        .site-expander summary::before {
            content: '>';
            font-size: 0.7rem;
            margin-right: 0.5rem;
            transition: transform 0.2s;
        }
        .site-expander[open] summary::before { transform: rotate(90deg); }
        .site-content {
            padding: 1rem;
            background: var(--color-surface);
            border: 1px solid var(--color-border);
        ...
        .defect-name { font-weight: 600; font-size: 0.9rem; }
        .defect-eqp { color: var(--color-text-muted); font-size: 0.8rem; }
        .defect-duration { color: #94a3b8; font-size: 0.75rem; font-style: italic; margin-top: 0.25rem; }
        ...
        @media (max-width: 768px) {
            .grid-2, .filter-row-errors { grid-template-columns: 1fr; }
...
<body>
    <header class="header">
        <div class="header-logo">ELTO</div>
        <div class="header-center">
            <div class="header-title">Dashboard Erreurs de Charge</div>
            <img src="{{ asset('assets/nidec.png') }}" alt="Nidec Logo" class="header-nidec-logo">
        </div>
        <div style="display: flex; align-items: center; gap: 1rem;">
            ...
            <div>Mise a jour: <span id="update-time">--:--</span></div>
    ...
    <main class="main">
        <section class="filters">
            ...
            <div class="filter-row">
                <button class="btn" onclick="toggleAllSites()" id="btn-all-sites">Tous les sites</button>
                <div class="tags-box" id="sites-tags"></div>
            </div>

            <!-- Periode -->
            <div class="filter-label">Periode d'analyse</div>
            <div class="filter-row period-btns">
                ...
                <button class="btn" data-mode="toute_periode" onclick="setDateMode('toute_periode')">Toute la periode</button>
            </div>

            <!-- Month selector -->
            <div class="filter-row month-selector" id="month-selector">
                <div>
                    <div class="filter-label">Annee</div>
            ...
            <div class="filter-row" id="day-selector" style="display:none;">
                <div>
                    <div class="filter-label">Selectionner une date</div>
                    <input type="date" class="select-input" id="day-input" onchange="updateDates()">
                </div>
            </div>

            <!-- Period summary -->
            <div class="summary" id="period-summary">Mois complet : December 2025</div>

            <!-- Global summary -->
            <div class="summary-bar">
                <span><strong>Periode:</strong> <span id="sum-dates">--</span></span>
            ...
            <div class="filter-row-errors">
                ...
                <div class="toggles">
                    <div class="toggle-row">
                        ...
                        <span class="toggle-label">Avant charge</span>
                    </div>
                    <div class="toggle-row">
                        <label class="toggle">
                        ...
                        <span class="toggle-label">Charge</span>
                    </div>
                    <div class="toggle-row">
                        <label class="toggle">
                        ...
                        <span class="toggle-label">Fin de charge</span>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="tabs-section">
            <div class="tabs-nav">
                ...
                <button class="tab-btn" data-tab="general">Generale</button>
                <button class="tab-btn" data-tab="details-site">Details Site</button>
                <button class="tab-btn" data-tab="comparaison">Comparaison</button>
                <button class="tab-btn" data-tab="stats">Statistiques</button>
                <button class="tab-btn" data-tab="analyse-erreur">Analyse Erreur</button>
                <button class="tab-btn" data-tab="code-analysis">Codes</button>
                <button class="tab-btn" data-tab="projection">Projection</button>
                <button class="tab-btn" data-tab="mac-address">MAC Address</button>
                <button class="tab-btn" data-tab="tentatives">Multi-tentatives</button>
                <button class="tab-btn" data-tab="suspectes">Transactions < 1 kWh</button>
                <button class="tab-btn" data-tab="alertes">Erreurs recurrentes</button>
    ...
    <script>
        ...
        function renderSites() {
            document.getElementById('sites-tags').innerHTML = state.sites.map(s =>
                `<span class="tag">${s} <button class="tag-remove" onclick="removeSite('${s}')">x</button></span>`
            ).join('');
            document.getElementById('btn-all-sites').textContent =
                state.sites.length === state.allSites.length ? 'Tous les sites' : 'Tous les sites';
        ...
        function renderErrorTypes() {
            document.getElementById('error-type-tags').innerHTML = state.errorTypes.map(t => {
                ...
                return `<span class="tag ${isSelected ? '' : 'inactive'}" onclick="toggleErrorType('${t}')">${label} <button class="tag-remove" onclick="event.stopPropagation(); toggleErrorType('${t}')">${isSelected ? 'x' : '+'}</button></span>`;
            }).join('');
        }
        ...
        function renderMoments() {
            document.getElementById('moment-tags').innerHTML = state.moments.map(m => {
                ...
                return `<span class="tag ${isSelected ? '' : 'inactive'}" onclick="toggleMoment('${m}')">${m} <button class="tag-remove" onclick="event.stopPropagation(); toggleMoment('${m}')">${isSelected ? 'x' : '+'}</button></span>`;
            }).join('');
        }
        ...
        function updateToggles() {
            ...
            document.getElementById('toggle-fin').checked = hasFin;
        }
        ...
        function updateDates() {
            ...
            switch(state.dateMode) {
                ...
                case 'semaine_minus_1':
                    ...
                    break;
                case 'toute_periode':
                    state.dateDebut = '{{ $date_min ?? "2024-01-01" }}';
                    state.dateFin = '{{ $date_max ?? "2025-12-31" }}';
                    break;
            }
            updateSummary();
            onFiltersChange();
        ...
        function updateSummary() {
            ...
            switch(state.dateMode) {
                ...
                case 'toute_periode': text = `Toute la periode : ${state.dateDebut} -> ${state.dateFin}`; break;
            }
            document.getElementById('period-summary').textContent = text;
            document.getElementById('sum-dates').textContent = `${state.dateDebut} -> ${state.dateFin}`;
        }
